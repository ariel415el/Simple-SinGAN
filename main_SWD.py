...
def main(image_paths, opt):
    ...
    for lvl in range(opt.num_levels):
        ...
        netG, shape, noise_amp, cur_lvl_fixed_z = train_single_scale(multi_scale_generator, netG, reference_pyramid, fixed_zs, opt)
...
def train_single_scale(multi_scale_generator, netG, reference_pyr, prev_fixed_zs, opt):
    ...
    criteria = PatchSWDLoss(patch_size=3, num_proj=64)
    if lvl == 0:
        noise_amp = 1
    else:
        with torch.no_grad():
